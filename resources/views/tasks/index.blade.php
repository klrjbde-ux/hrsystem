...
<script>
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
            ...
            if (commentButtons.length && commentsBody && sendCommentBtn && newCommentInput) {
                ...
                <div class="comment-actions">
                    ...
                    <button class="replyComment btn btn-sm btn-info"
                        data-id="${comment.id}"
                        data-user="${comment.user_name}"
                        data-text="${encodeURIComponent(comment.comment)}">
                ...
                `;
                                ...
                                if (comment.user_id == currentUserId) {
                                    ...
                                    actionButtons += `
                        <button class="editComment edit-btn"
                            data-id="${comment.id}">
                            <i class="bi bi-pencil"></i>
                        </button>

                        <button class="deleteComment delete-btn"
                            data-id="${comment.id}">
    ...
    <small>${comment.user_name}</small><br>
    ${replyHtml}
    <span class="commentText">${comment.comment}</span>
    ${comment.is_edited ? `<button class="btn btn-sm btn-link p-0 edited-comment" data-comment-id="${comment.id}">(edited)</button>` : ''}
    ${actionButtons}
`;
            ...
            document.addEventListener('click', function(e) {
                ...
                saveBtn.addEventListener('click', () => {
                    ...
                    fetch(`/comments/${commentId}`, {
                        ...
                        .then(data => {
                            if (data.success) {
                                // Update text and show (edited)
                                commentTextSpan.innerHTML = newText + ' <small class="text-muted ms-1">(edited)</small>';
        ...
        <div class="alert alert-light border p-2">
            ...
            <button class="btn btn-sm btn-danger float-end" id="cancelReply">x</button>
        </div>
    `;
            });
