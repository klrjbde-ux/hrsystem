  ...
  <div class="row">
    <div class="col-lg-12">
        <div class="card">
                    <div class="card-body">
                      @if(isset($salary->id))
                      <form class="row g-3 needs-validation" validate action="{{route('updateSalary', $salary->id)}}" method="POST">
                        @csrf
                        
                        <!-- Employee Data -->
                        <div class="row col-12 gap">
                          <div class="col-md-6">
                              <label for="bonus" class="form-label">Bonus<span class="text-danger">*</span></label>
                              <input type="number" name="bonus" class="form-control" id="bonus"
                              oninput="checkLength(this)"  placeholder="Enter Amount" >
                              @error('bonus')                        
    ...
    <section class="section">
      <div class="row">
       <div class="col-lg-2"></div>
        <div class="col-lg-7" style="margin-left: 74px">
                    ...
                    <table class="table table-responsive-lg table-responsive-sm table-responsive-md">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" style="font-size: 15px">Bonus</th>                             
                        ...
                        <tbody>
                            ...
                            <div class="employee-card">
                              @foreach ($salaryBonus as $salaryBonus)
                           
                            <tr>
                              <th scope="row">{{ $loop->iteration }}</th>
                              <td>{{ $salaryBonus->month }}</td>
                            
                               <!-- Assuming each salary deduction has a relationship to the deduction type -->
                             @foreach ($bonus_types as $bonus_type)
                             @if ($bonus_type->id === $salaryBonus->bonus_type) <!-- Match deduction types -->
                             <td>{{ $bonus_type->status }}</td>
                               @endif
                              @endforeach
                             
                             
                              <td>{{ $salaryBonus->bonus_reason }}</td>
                              <td>{{ $salaryBonus->bonus_amount}}</td>
                             <td>  <a href="{{ route('bonusdelete', $salaryBonus->id)}}"><i class="fa fa-trash" 
